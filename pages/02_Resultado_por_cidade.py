...
if __name__ == "__main__":
    df_origin = dados_df()
    cidade = st.selectbox("Escolha uma cidade", df_origin.cidade.unique())
    df = df_origin.query(f"cidade == '{cidade}'")
   
    f"""# Respostas Gerais

Foram coletadas {df.shape[0]} respostas da cidade de {cidade}"""
...
"""
        ...
        "## Cursos"
        interessados = df.query("interesse == 'Sim' or continuar == 'Sim'")
    ...
    with tab_tabelas: 
        ...
        grau_intrucao_total = pd.DataFrame(data=[["Total", graun_instrucao.contagem.sum()]], columns=graun_instrucao.columns)
        grau_instrucao_full = pd.concat([graun_instrucao, grau_intrucao_total], ignore_index=True) 
        "## Grau de instrução"
        st.dataframe(grau_instrucao_full)
